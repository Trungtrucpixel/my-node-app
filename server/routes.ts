...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/profit-sharings/process", async (req, res) => {
    ...
    try {
      ...
      const { period, periodValue, forceReprocess } = validationResult.data;
      
      console.log(`🔄 Processing profit sharing for ${periodValue} (force: ${forceReprocess})`);
      
      const profitSharing = await storage.processQuarterlyProfitSharing(period, periodValue, forceReprocess);
